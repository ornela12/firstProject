<div class="col-lg-8">
    <div class="panel panel-default" style="height:50px;">
		<span style="font-size:18px; margin-left:10px; position:relative; top:13px;"><strong> List of Friend Chats</strong></span>
	...
	<table width="100%" class="table table-striped table-bordered table-hover" id="chatRoom">
		...
		<tbody>
		<?php
			$query=mysqli_query($link,"select * from chatroom order by date_created desc");
			...
			<tr>
				...
				<?php
				$usera=array();
				$m=mysqli_query($link,"select * from chat_member where chatroomid='".$row['chatroomid']."'");
				...
				<td>
					<?php
					$num=mysqli_query($link,"select * from chat_member where chatroomid='".$row['chatroomid']."'");
					?>
				...
				<td><button value="<?php echo $row['chatroomid']; ?>" class="btn btn-info join_chat">Join</button> &nbsp;
					...
					$qq=mysqli_query($link,"select * from chat_member where chatroomid='".$row['chatroomid']."' and userid='".$_SESSION['user_id']."'");
