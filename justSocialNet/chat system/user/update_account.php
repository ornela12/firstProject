<?php
	...
	$myq=mysqli_query($conn,"select * from `users` where user_id='".$_SESSION['id']."'");
	...
	elseif ($cpassword!=$myqrow['password']){
		...
		mysqli_query($conn,"update `user` set username='$musername', password='$newpassword', uname='$mname' where user_id='".$_SESSION['id']."'");
		?>
		<script>
